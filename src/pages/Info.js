...
function Info() {
  ...
  const handleSetIdRecently = useCallback(
    (chapters, comic) => {
      if (!comic && !isSort) {
  ...
  const handleSetIdStorage = (comic) => {
    const chapterIds = comic?.map((chapter) => chapter?.data?.item?._id) || [];
    return chapterIds;
  };
  ...
  }, [data, handleSetIdRecently, params?.slug, user?.email, width]);
  ...
  useEffect(() => {
    ...
    let newChapters =
      width > 1024
        ? chapters.slice(0, chapterInDesktop)
        : chapters.slice(0, chapterInMobile);
    setChapters(isCollapse ? chapters : newChapters);
    setValueSearchChapter("");
  }, [data?.data?.item?.chapters, isCollapse, width]);
  ...
  const handleSearchChapter = (event) => {
    ...
    const filterChapters = dataChapter.filter((chapter) =>
  ...
  const handleSaveComic = () => {
    ...
    width > 1023 && setQuantityComicArchive(comicStorage[user?.email].length);
    setIsSave(true);
  ...
  const handleDeleteComic = () => {
    ...
    comicStorage[user?.email] = comicStorage[user?.email].filter(
      (comic) => comic?.slug !== params?.slug
    );
    storage.set("comic-storage", comicStorage);
    setIsSave(false);
          ...
          <div className="lg:flex lg:flex-row mobile:flex-col mobile:gap-[12px] p-[16px] bg-[rgba(16,185,129,0.15)] rounded-[8px] dark:bg-[rgba(204,204,204,0.2)]">
            ...
            <div className="overflow-hidden flex flex-col gap-[4px] flex-1 lg:pl-[32px]">
              ...
              <div className="flex gap-[12px] my-[12px] select-none">
                ...
                <Link
                  className={`py-[4px] px-[12px] mobile:px-[8px] rounded-[8px] block text-lg font-[600] transition-all hover:scale-[1.05] text-[#fff] ${
                    chapters.length > 0
                      ? "bg-[#3b82f6] pointer-events-auto"
                      : "bg-[rgba(59,131,246,0.43)] pointer-events-none cursor-none"
                  }`}
                  to={`/read/${params.slug}/${idRecently}`}
                >
              ...
              <div className="flex gap-[12px] text-lg">
                ...
                <span className="dark:text-[#fff]">
                  {formatDate(infoComic?.updatedAt)}
                </span>
              </div>
              <ul className="flex gap-[12px] text-lg flex-wrap">
                ...
                {category.map((category, index) => (
                  <li key={index} className="transition-all hover:scale-[1.05]">
                    <Link
                      className="px-[6px] py-[4px] bg-[#10b981] rounded-[8px] text-sm text-[#fff]"
              ...
              {chapters.length > 0 ? (
                <Fragment>
                    ...
                    <li className="xl:w-chapter-desktop lg:w-chapter-table mobile:w-chapter-mobile">
                      <Link
                        ...
                        to={`/read/${
                          ...
                          data?.data?.item?.chapters?.[0]?.server_data.length -
                            1
                        ]?.chapter_api_data
                          .split("/")
                          .pop()}`}
                  ...
                  {chapters.map((chapter, index) => (
                    <li
                      className={` transition-all hover:scale-[1.05] rounded-[8px] xl:w-chapter-desktop   lg:w-chapter-table mobile:w-chapter-mobile relative ${
                        idStorage.includes(
                ...
                <span className="font-[600] text-xl text-[#d90429]">
                  Không có dữ liệu!
                </span>
              )}
            </ul>
